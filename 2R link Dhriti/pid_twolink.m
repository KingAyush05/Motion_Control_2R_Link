...
xlabel('time');
ylabel('theta');
plot(t,(yd(1))*ones(length(t),1),'--b','DisplayName','theta1d');
plot(t,y(:,1),'-b','DisplayName','theta1');
plot(t,(yd(3))*ones(length(t),1),'--r','DisplayName','theta2d');
...
e1 = thetad1 - y(1);
e2 = thetad2 - y(3);
T1 = Kp1*e1 + Kd1*(omega1d - y(2));
T2 = Kp2*e2 + Kd2*(omega2d - y(4));
% Equations to solve
dydt = [y(2)
        ...
        y(6)*y(2)+y(7)*y(4)
        Kp1*omega1d + Ki1*(theta1d - y(1))
        Kp2*omega2d + Ki2*(theta2d - y(3))];
end